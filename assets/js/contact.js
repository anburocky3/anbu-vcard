  ...
  ;(function () {
    ...
    function _bindEvents() {
      ...
      $contactForm.live('submit', function () {
        var status =
          _nameValidate(true) & _emailValidate(true) & _textValidate(true)
        if (!!status) {
          _submitForm()
        }
        return false
        ...
        success: function (msg) {
          if (msg === 'SEND') {
            ...
            setTimeout(function () {
              ...
              $textField.val('')
              $contactForm
                .find('label[for="' + $nameField.attr('id') + '"]')
                .css('display', 'block')
                .css('opacity', 1)
              $contactForm
                .find('label[for="' + $emailField.attr('id') + '"]')
                .css('display', 'block')
                .css('opacity', 1)
              $contactForm
                .find('label[for="' + $textField.attr('id') + '"]')
                .css('display', 'block')
                .css('opacity', 1)
            }, 1000)
          } else {
